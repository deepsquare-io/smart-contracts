...
contract Ballot is Ownable {
    ...
    function init(string memory _subject, uint32 _tagIndex, string[] memory _choices) external onlyOwner {
        subject = _subject;
        tagIndex = _tagIndex;
        closed = false;
        choices = _choices;
    ...
    function closeBallot() external onlyOwner {
        ...
        for(uint i = 0; i < voters.length; i++) { // if A has granted proxy to B
            ...
            resultStorage[votes[voter].choiceIndex] += DPS.balanceOf(msg.sender) + proxy.proxyAmount(voter, tagIndex);
        }
    }
