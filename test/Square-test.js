...
describe('Square', function () {
  ...
  beforeEach(async function () {
    ...
    fakeUSDTe = await FakeUSDTe.deploy()
    await fakeUSDTe.deployed()
    // Deploy Square
    const Square = await ethers.getContractFactory('SquareFundRaiser')
    square = await Square.deploy(owner.address, fakeUSDTe.address)
    ...
    await fakeUSDTe.transfer(addr1.address, inToken(10000))
    await fakeUSDTe.transfer(addr2.address, inToken(8000))
  })
  ...
  it('User participates to fundraising after setting reference', async function () {
    ...
    await fakeUSDTe.connect(addr1).approve(square.address, inToken(500))
    ...
    await fakeUSDTe.connect(addr1).approve(square.address, inToken(1501))
    ...
    // Increase limit
    await fakeUSDTe.connect(addr1).approve(square.address, 0)
    await fakeUSDTe.connect(addr1).approve(square.address, inToken(1000))
    const nextPhaseAmount = inToken(10e5)
    square.setNextFundingCap(nextPhaseAmount)
    expect(await square.nextFundingCap()).to.equal(nextPhaseAmount)
    // Increase Price
    ...
    await fakeUSDTe.connect(addr2).approve(square.address, inToken(0))
    await fakeUSDTe
      .connect(addr2)
